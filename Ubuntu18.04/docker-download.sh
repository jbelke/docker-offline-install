...
apt-get update
apt-get -y install apt-transport-https ca-certificates apt-mirror curl gnupg-agent software-properties-common

# Get Docker's public key and add it to apt for package signiture verification
#apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 58118E89F3A912897C070ADBF76221572C52609D

## WIP: update add-apt-repository
#curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add -
sudo sh docker-key-download.sh
...
#echo deb https://apt.dockerproject.org/repo ubuntu-bionic main > /etc/apt/mirror.list
#echo deb https://apt.dockerproject.org/repo ubuntu-bionic main > /etc/apt/mirror.list
